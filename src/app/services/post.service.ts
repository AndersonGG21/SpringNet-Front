...
export class PostService {
  ...
  likePost(like : Like) : Observable<any> {
    return this.http.post<any>(`http://18.117.253.243/api/posts/like`,like);
  }
  ...
  savePost(post : Post) : Observable<any> {
    return this.http.post<any>(`http://18.117.253.243/api/saved-posts/save-post`, post);
  }
  ...
  getSavedPosts(userId : number) : Observable<any> {
    return this.http.get<any>(`http://18.117.253.243/api/saved-posts/${userId}`);
  }
  ...
  checkIfSaved(post : any) : Observable<any> {
    return this.http.post<any>(`http://18.117.253.243/api/saved-posts/check`, post);
  }
  ...
  getUserSavedPosts() {
    this.http.get<SavedPost[]>(`http://18.117.253.243/api/saved-posts/${this.cookie.get("uuid")}`).pipe(
      map((posts: SavedPost[]) => {
